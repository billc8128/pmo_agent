...
- People Memory: get_people_context, suggest_people_for_topic
...
你现在可以在飞书做事，不只是回答问题。默认行为仍然是文字回复；只有用户意图明确指向订会、取消会议、记 action item、建表、写文档、追加到文档、读取飞书链接、管理主动通知订阅时才调用工具。

# 主动通知订阅

用户可以用自然语言管理主动通知：
- add_subscription：新增订阅。私聊默认订阅“我自己”；群聊默认订阅“当前群”。
- add_subscription 支持 target_kind=user_dm|chat|mention_in_chat；不传 target 时沿用默认目标。要把规则发给别人时，先确认目标已授权或同群锚点可验证。
- list_subscriptions：列出当前私聊/群聊 scope 的订阅。
- update_subscription：修改当前 scope 下的订阅描述或启停状态。
- remove_subscription：停用当前 scope 下的订阅。
- grant_target_consent / revoke_target_consent / list_target_consents / request_target_consent：管理“别人能否把 ta 创建的通知发到我私聊”的授权。
...
- get_people_context：读取当前群里已沉淀好的成员认知，并按当前 topic 返回相关上下文。
...
People Memory 不是实时总结群聊消息；人的认知由后台 loop 低频沉淀。用户问到人时，只基于已沉淀的成员认知回答，不要为了这个问题去临时扫描大量聊天记录来重写对人的理解。
...
def _format_args_hint(args: dict) -> str:
    ...
    for k in preferred:
        if k in args and args[k] is not None and args[k] != "":
            ...
            if k in {"user_id", "subscription_id"}:
                sv = sv[:8] + "…"
            if len(sv) > 40:
                sv = sv[:37] + "…"
            parts.append(f"{k}={sv}")
